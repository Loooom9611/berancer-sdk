...
var TokenAmount = class _TokenAmount {
  ...
  toSignificant(significantDigits = 6) {
    let result = new _Decimal(this.amount.toString()).div(new _Decimal(this.decimalScale.toString())).toDecimalPlaces(significantDigits).toFixed(significantDigits);
    if (result.includes(".")) {
      while (result.at(-1) === "0") {
...
var InitPoolDataProvider = class {
  constructor(chainId, rpcUrl) {
    ...
    this.client = createPublicClient({
      transport: http(this.rpcUrl, { timeout: 6e4 }),
  ...
  async getInitPoolDataV2(poolAddress, poolType) {
    const poolContract = getContract2({
 ...
 */
__publicField(_ComposableStableEncoder, "initPool", (amountsIn) => encodeAbiParameters2(
  [{ type: "uint256" }, { type: "uint256[]" }],
 ...
 */
__publicField(_ComposableStableEncoder, "addLiquidityUnbalanced", (amountsIn, minimumBPT) => encodeAbiParameters2(
 ...
 */
__publicField(_ComposableStableEncoder, "removeLiquidityUnbalanced", (amountsOut, maxBPTAmountIn) => encodeAbiParameters2(
  [{ type: "uint256" }, { type: "uint256[]" }, { type: "uint256" }],
  [
    BigInt(
...
var getPoolStateWithBalancesCowAmm = async (poolState, chainId, rpcUrl) => {
  ...
  const poolStateWithBalances = {
    ...
    tokens: sortedTokens.map((token, i) => ({
      ...
      decimals: token.decimals,
      index: i,
      balance: formatUnits(balances[i], token.decimals)
    })),
...
function calculateProportionalAmountsCowAmm(pool, referenceAmount) {
  ...
  const balances = tokensWithBpt.map(
    (t) => parseUnits2(t.balance, t.decimals)
  );
...
function getAmounts(tokens, amounts, defaultAmount = 0n) {
  return tokens.map(
    (t) => amounts.find((a) => t.isSameAddress(a.address))?.rawAmount ?? defaultAmount
  );
}
...
var getPoolStateWithBalancesV2 = async (poolState, chainId, rpcUrl) => {
  ...
  const poolStateWithBalances = {
    ...
    tokens: sortedTokens.map((token, i) => ({
      ...
      decimals: token.decimals,
      index: i,
      balance: formatUnits2(balances[i], token.decimals)
    })),
    totalShares: formatEther2(totalShares)
  };
  return poolStateWithBalances;
};
...
var getPoolStateWithBalancesV3 = async (poolState, chainId, rpcUrl) => {
  const publicClient = createPublicClient6({
  ...
  const poolStateWithBalances = {
    ...
    tokens: tokenAmounts.map((tokenAmount, i) => ({
      ...
      index: i,
      balance: formatUnits3(
        tokenAmount.amount,
        tokenAmount.token.decimals
      )
    })),
    totalShares: formatEther3(totalShares)
  };
  return poolStateWithBalances;
};
...
function calculateProportionalAmounts(pool, referenceAmount) {
  ...
  const balances = tokensWithBpt.map(
    (t) => parseUnits3(t.balance, t.decimals)
  );
  const referenceTokenBalance = balances[referenceTokenIndex];
...
function isPoolToken(tokens, token) {
  ...
  tokens.some((t) => {
    ...
    const isUnderlying = isPoolTokenWithUnderlying(t) && t.underlyingToken?.address && isSameAddress(t.underlyingToken.address, token);
 ...
 * @param initialBalances - the amounts of tokens to send to the pool to form the initial balances
 */
__publicField(_StableEncoder, "initPool", (amountsIn) => encodeAbiParameters3(
  [{ type: "uint256" }, { type: "uint256[]" }],
  [BigInt(0 /* INIT */), amountsIn]
));
/**
 ...
 * @param minimumBPT - the minimum acceptable BPT to receive in return for deposited tokens
 */
__publicField(_StableEncoder, "addLiquidityUnbalanced", (amountsIn, minimumBPT) => encodeAbiParameters3(
  [{ type: "uint256" }, { type: "uint256[]" }, { type: "uint256" }],
  [
 ...
 * @param maxBPTAmountIn - the minimum acceptable BPT to burn in return for withdrawn tokens
 */
__publicField(_StableEncoder, "removeLiquidityUnbalanced", (amountsOut, maxBPTAmountIn) => encodeAbiParameters3(
  [{ type: "uint256" }, { type: "uint256[]" }, { type: "uint256" }],
  [
 ...
 * @param initialBalances - the amounts of tokens to send to the pool to form the initial balances
 */
__publicField(_WeightedEncoder, "initPool", (amountsIn) => encodeAbiParameters4(
  [{ type: "uint256" }, { type: "uint256[]" }],
  [BigInt(0 /* INIT */), amountsIn]
));
/**
 ...
 * @param minimumBPT - the minimum acceptable BPT to receive in return for deposited tokens
 */
__publicField(_WeightedEncoder, "addLiquidityUnbalanced", (amountsIn, minimumBPT) => encodeAbiParameters4(
  [{ type: "uint256" }, { type: "uint256[]" }, { type: "uint256" }],
  [
    BigInt(1 /* EXACT_TOKENS_IN_FOR_BPT_OUT */),
    amountsIn,
 ...
 * @param maxBPTAmountIn - the minimum acceptable BPT to burn in return for withdrawn tokens
 */
__publicField(_WeightedEncoder, "removeLiquidityUnbalanced", (amountsOut, maxBPTAmountIn) => encodeAbiParameters4(
  [{ type: "uint256" }, { type: "uint256[]" }, { type: "uint256" }],
  [
    BigInt(2 /* BPT_IN_FOR_EXACT_TOKENS_OUT */),
    amountsOut,
...
import { encodeFunctionData as encodeFunctionData4, zeroAddress } from "viem";

// src/entities/addLiquidity/addLiquidityV3/doAddLiquidityUnbalancedQuery.ts
import { createPublicClient as createPublicClient7, http as http7 } from "viem";
var doAddLiquidityUnbalancedQuery = async (rpcUrl, chainId, sender, userData, poolAddress, maxAmountsIn) => {
  const client = createPublicClient7({
    transport: http7(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient8, http as http8 } from "viem";
var doAddLiquiditySingleTokenQuery = async (rpcUrl, chainId, sender, userData, tokenIn, poolAddress, bptOut) => {
  const client = createPublicClient8({
    transport: http8(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient9, http as http9 } from "viem";
  ...
  validateCreatePool(input) {
    ...
    if (input.protocolVersion === 3) {
      input.tokens.forEach(({ tokenType, rateProvider }) => {
        if (tokenType !== 0 /* STANDARD */ && rateProvider === zeroAddress2) {
          throw new Error(
...
import { encodeFunctionData as encodeFunctionData6, zeroAddress as zeroAddress3 } from "viem";

// src/entities/addLiquidityBoosted/doAddLiquidityUnbalancedQuery.ts
import { createPublicClient as createPublicClient10, http as http10 } from "viem";
var doAddLiquidityUnbalancedQuery2 = async (rpcUrl, chainId, sender, userData, poolAddress, exactUnderlyingAmountsIn) => {
  const client = createPublicClient10({
    transport: http10(rpcUrl),
    chain: CHAINS[chainId]
  });
  const { result: bptAmountOut } = await client.simulateContract({
...
import { createPublicClient as createPublicClient11, http as http11 } from "viem";
var doAddLiquidityProportionalQuery2 = async (rpcUrl, chainId, sender, userData, poolAddress, exactBptAmountOut) => {
  const client = createPublicClient11({
    transport: http11(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient12, http as http12 } from "viem";
var doAddLiquidityQuery2 = async (rpcUrl, chainId, wrappedToken, exactSharesToIssue) => {
  const client = createPublicClient12({
    transport: http12(rpcUrl),
    chain: CHAINS[chainId]
  });
  const {
...
import {
  concat as concat2,
  encodeFunctionData as encodeFunctionData8,
  hexToBigInt,
  pad as pad2,
  slice,
  toHex as toHex2
...
var _Relayer = class _Relayer {
  ...
  static toChainedReference(key, isTemporary = true) {
    ...
    const paddedKey = pad2(toHex2(key), { dir: "left", size: 30 });
    const chainedReferenceWithPrefix = concat2([prefix, paddedKey]);
    return hexToBigInt(chainedReferenceWithPrefix);
  }
  static fromChainedReference(ref) {
    const chainedReferenceWithoutPrefix = slice(toHex2(ref), 2);
...
__publicField(_Relayer, "signRelayerApproval", async (relayerAddress, signerAddress, client) => {
  const approval = encodeFunctionData8({
    abi: vaultV2Abi,
    functionName: "setRelayerApproval",
...
var encodeCalls = (callsAttributes) => {
  ...
  for (const callAttributes of callsAttributes) {
    ...
    const encodedCall = encodeFunctionData9({
      abi: batchRelayerLibraryAbi,
      functionName: "joinPool",
      args: [
        poolId,
...
import {
  createPublicClient as createPublicClient13,
  decodeAbiParameters,
  decodeFunctionResult,
  http as http13
} from "viem";
var doAddLiquidityNestedQuery = async (chainId, rpcUrl, encodedMulticall) => {
  const client = createPublicClient13({
    transport: http13(rpcUrl),
    chain: CHAINS[chainId]
  });
...
var AddLiquidityNestedV2 = class {
  async query(input, nestedPoolState) {
    ...
    const encodedMulticall = encodeFunctionData10({
...
import {
  createPublicClient as createPublicClient14,
...
var AddLiquidityNestedV3 = class {
  constructor() {
    __publicField(this, "doQueryAddLiquidityUnbalancedNestedPool", async ({ rpcUrl, chainId }, parentPool, tokensIn, maxAmountsIn, sender, userData) => {
      const client = createPublicClient14({
        transport: http14(rpcUrl),
        chain: CHAINS[chainId]
      });
      const { result: bptAmountOut } = await client.simulateContract({
        address: BALANCER_COMPOSITE_LIQUIDITY_ROUTER[chainId],
...
import { encodeFunctionData as encodeFunctionData12, parseEther } from "viem";
...
var CreatePoolComposableStableV2 = class {
  buildCall(input) {
    ...
    const encodedCall = encodeFunctionData12({
...
import { encodeFunctionData as encodeFunctionData13, parseEther as parseEther2 } from "viem";
var CreatePoolWeightedV2 = class {
  buildCall(input) {
    const args = this.parseCreateFunctionArgs(input);
    const encodedCall = encodeFunctionData13({
...
import { encodeFunctionData as encodeFunctionData14, zeroAddress as zeroAddress5 } from "viem";
...
import { encodeFunctionData as encodeFunctionData15, zeroAddress as zeroAddress6 } from "viem";
...
import { createPublicClient as createPublicClient15, http as http15 } from "viem";
var doInitBufferQuery = async (rpcUrl, chainId, wrappedToken, exactAmountUnderlyingIn, exactAmountWrappedIn) => {
  const client = createPublicClient15({
    transport: http15(rpcUrl),
    chain: CHAINS[chainId]
  });
...
var InitBufferV3 = class {
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData16({
      abi: balancerBufferRouterAbi,
...
var InitPoolComposableStable = class {
  buildCall(input, poolState) {
    ...
    const callData = encodeFunctionData17({
      abi: vaultV2Abi,
      functionName: "joinPool",
      args
    });
...
var InitPoolWeighted = class {
  buildCall(input, poolState) {
    ...
    const callData = encodeFunctionData18({
      abi: vaultV2Abi,
      functionName: "joinPool",
      args
    });
...
var InitPoolV3 = class {
  buildCall(input, poolState) {
    ...
    const callData = encodeFunctionData19({
      abi: balancerRouterAbi,
...
__publicField(PermitHelper, "signRemoveLiquidityApproval", async (input) => {
  ...
  const nonce = input.nonce ?? await getNonce(
    input.client,
...
import { createPublicClient as createPublicClient16, http as http16 } from "viem";
async function doRemoveLiquidityQuery(rpcUrl, chainId, args) {
  const client = createPublicClient16({
    transport: http16(rpcUrl),
    chain: CHAINS[chainId]
  });
  const {
...
var RemoveLiquidityWeighted = class {
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData20({
      abi: vaultV2Abi,
...
var RemoveLiquidityComposableStable = class {
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData21({
      abi: vaultV2Abi,
      functionName: "exitPool",
      args
    });
...
var RemoveLiquidityStable = class {
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData22({
      abi: vaultV2Abi,
      functionName: "exitPool",
      args
    });
...
import { createPublicClient as createPublicClient17, http as http17 } from "viem";
var doRemoveLiquiditySingleTokenExactOutQuery = async (rpcUrl, chainId, sender, userData, poolAddress, tokenOut, exactAmountOut) => {
  const client = createPublicClient17({
    transport: http17(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient18, http as http18 } from "viem";
var doRemoveLiquiditySingleTokenExactInQuery = async (rpcUrl, chainId, sender, userData, poolAddress, tokenOut, exactBptAmountIn) => {
  const client = createPublicClient18({
    transport: http18(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient19, http as http19 } from "viem";
var doRemoveLiquidityProportionalQuery = async (rpcUrl, chainId, sender, userData, poolAddress, exactBptAmountIn) => {
  const client = createPublicClient19({
    transport: http19(rpcUrl),
    chain: CHAINS[chainId]
  });
...
import { createPublicClient as createPublicClient20, http as http20 } from "viem";
var doRemoveLiquidityRecoveryQuery = async ({ chainId, rpcUrl, bptIn }, poolAddress) => {
  const client = createPublicClient20({
    transport: http20(rpcUrl),
    chain: CHAINS[chainId]
  });
  const { result: amountsOut } = await client.simulateContract({
    address: BALANCER_ROUTER[chainId],
...
import { encodeFunctionData as encodeFunctionData27, zeroAddress as zeroAddress7 } from "viem";
var RemoveLiquidityV3 = class {
  ...
  buildCallWithPermit(input, permit) {
    ...
    const args = [
      permit.batch,
      permit.signatures,
      { details: [], spender: zeroAddress7, sigDeadline: 0n },
      "0x",
      [buildCallOutput.callData]
    ];
    const callData = encodeFunctionData27({
      abi: balancerRouterAbi,
      functionName: "permitBatchAndCall",
      args
    });
...
var RemoveLiquidityCowAmm = class {
  ...
  buildCall(input) {
    ...
    const amounts = getAmountsCall2(input);
    const callData = encodeFunctionData28({
      abi: cowAmmPoolAbi,
      functionName: "exitPool",
...
import { encodeFunctionData as encodeFunctionData30 } from "viem";

// src/entities/removeLiquidityNested/removeLiquidityNestedV2/encodeCalls.ts
import { encodeFunctionData as encodeFunctionData29 } from "viem";
var encodeCalls2 = (callsAttributes, isProportional) => {
  ...
  for (const callAttributes of callsAttributes) {
    ...
    const encodedCall = encodeFunctionData29({
      abi: batchRelayerLibraryAbi,
      functionName: "exitPool",
      args: [
        poolId,
...
import {
  createPublicClient as createPublicClient21,
...
var doRemoveLiquidityNestedQuery = async (chainId, rpcUrl, encodedMulticall, tokensOutIndexes) => {
  const client = createPublicClient21({
    transport: http21(rpcUrl),
    chain: CHAINS[chainId]
  });
  const { data } = await client.call({
    ...EMPTY_SENDER,
  ...
  result.forEach((r, i) => {
    if (tokensOutIndexes.includes(i))
      peekedValues.push(decodeAbiParameters2([{ type: "uint256" }], r)[0]);
  });
  return peekedValues;
};
...
var RemoveLiquidityNestedV2 = class {
  async query(input, nestedPoolState) {
    ...
    const encodedMulticall = encodeFunctionData30({
      abi: balancerRelayerAbi,
      functionName: "vaultActionsQueryMulticall",
      args: [encodedCalls]
    });
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData30({
      abi: balancerRelayerAbi,
      functionName: "multicall",
      args: [encodedCalls]
    });
...
import {
  createPublicClient as createPublicClient22,
  encodeFunctionData as encodeFunctionData31,
  http as http22,
  zeroAddress as zeroAddress8
} from "viem";
var RemoveLiquidityNestedV3 = class {
  constructor() {
    __publicField(this, "doQueryRemoveLiquidityProportionalNestedPool", async ({ rpcUrl, chainId }, parentPool, exactBptAmountIn, tokensOut, sender, userData) => {
      const client = createPublicClient22({
        transport: http22(rpcUrl),
        chain: CHAINS[chainId]
      });
      const { result: amountsOut } = await client.simulateContract({
        address: BALANCER_COMPOSITE_LIQUIDITY_ROUTER[chainId],
  ...
  buildCall(input) {
    ...
    const callData = encodeFunctionData31({
      abi: balancerCompositeLiquidityRouterAbi,
...
import { encodeFunctionData as encodeFunctionData32, zeroAddress as zeroAddress9 } from "viem";
var RemoveLiquidityNested = class {
  ...
  buildCallWithPermit(input, permit) {
    ...
    const args = [
      permit.batch,
      permit.signatures,
      { details: [], spender: zeroAddress9, sigDeadline: 0n },
      "0x",
      [buildCallOutput.callData]
    ];
    const callData = encodeFunctionData32({
      abi: balancerCompositeLiquidityRouterAbi,
      functionName: "permitBatchAndCall",
      args
    });
...
function validateBufferVersion(paths) {
  if (!paths.every((p) => {
    return p.isBuffer?.some((b) => b === true) ? p.protocolVersion === 3 : true;
  })) {
...
import {
  createPublicClient as createPublicClient23,
  encodeFunctionData as encodeFunctionData33,
...
var SwapV2 = class {
  ...
  async query(rpcUrl, block) {
    const client = createPublicClient23({
      chain: CHAINS[this.chainId],
      transport: rpcUrl ? http23(rpcUrl) : http23()
    });
    const queriesContract = getContract3({
...
import {
  createPublicClient as createPublicClient24,
  encodeFunctionData as encodeFunctionData34,
  getContract as getContract4,
  http as http24,
  zeroAddress as zeroAddress10
} from "viem";
var SwapV3 = class {
  constructor({
    chainId,
  ...
  async query(rpcUrl, block, sender) {
    const client = createPublicClient24({
      chain: CHAINS[this.chainId],
      transport: rpcUrl ? http24(rpcUrl) : http24()
    });
  ...
  async querySingleSwap(client, block, sender) {
    const routerContract = getContract4({
  ...
  async queryBatchSwap(client, block, sender) {
    const batchRouterContract = getContract4({
  ...
  buildCallWithPermit2(input, permit2) {
    ...
    const callData = encodeFunctionData34({
      abi: balancerBatchRouterAbi,
      functionName: "permitBatchAndCall",
      args
    });
...
import { formatEther as formatEther4, parseEther as parseEther3 } from "viem";
var PriceImpactAmount = class _PriceImpactAmount {
  constructor(amount) {
    ...
    this.amount = BigInt(amount);
    this.decimal = parseFloat(formatEther4(this.amount));
  ...
  static fromDecimal(decimalAmount) {
    const rawAmount = parseEther3(decimalAmount);
...
import { encodeFunctionData as encodeFunctionData35, zeroAddress as zeroAddress11 } from "viem";

// src/entities/removeLiquidityBoosted/doRemoveLiquidityProportionalQuery.ts
import { createPublicClient as createPublicClient25, http as http25 } from "viem";
var doRemoveLiquidityProportionalQuery2 = async (rpcUrl, chainId, exactBptAmountIn, sender, userData, poolAddress) => {
  const client = createPublicClient25({
    transport: http25(rpcUrl),
    chain: CHAINS[chainId]
  });
...
var RemoveLiquidityBoostedV3 = class {
  ...
  buildCall(input) {
    const amounts = getAmountsCall2(input);
    const callData = encodeFunctionData35({
      abi: balancerCompositeLiquidityRouterAbi,
  ...
  buildCallWithPermit(input, permit) {
    ...
    const args = [
      permit.batch,
      permit.signatures,
      { details: [], spender: zeroAddress11, sigDeadline: 0n },
      "0x",
      [buildCallOutput.callData]
    ];
    const callData = encodeFunctionData35({
...
import { formatEther as formatEther5, parseEther as parseEther4 } from "viem";
var Slippage = class _Slippage {
  ...
  constructor(amount) {
    ...
    this.decimal = parseFloat(formatEther5(this.amount));
    this.percentage = this.decimal * 100;
    this.bps = this.decimal * 1e4;
  }
  static fromRawAmount(rawAmount) {
  ...
  static fromDecimal(decimalAmount) {
    const rawAmount = parseEther4(decimalAmount);
...
import { createPublicClient as createPublicClient26, http as http26 } from "viem";
...
import {
  ...
  encodeFunctionData as encodeFunctionData38,
...
import {
  ...
  encodeFunctionData as encodeFunctionData36,
  parseAbiParameters
} from "viem";
...
function encodeJoinData(token, sender, inputAmount, wethIsEth) {
  ...
  const poolKind = 1;
  const userData = encodeAbiParameters5(
  ...
  const joinPoolData = encodeFunctionData36({
    abi: batchRelayerLibraryAbi,
    functionName: "joinPool",
    args: [
      balWethId,
...
import { encodeFunctionData as encodeFunctionData37 } from "viem";
function encodeSwapData(amount, assetIn, assetOut, sender, recipient, limit, value, isTempRef) {
  ...
  const swapData = encodeFunctionData37({
    abi: batchRelayerLibraryAbi,
    functionName: "swap",
    args: [
      singleSwap,
...
async function queryJoinSwap(input) {
  ...
  const encodedMulticall = encodeFunctionData38({
    abi: balancerRelayerAbi,
    functionName: "vaultActionsQueryMulticall",
  ...
  const result = decodeFunctionResult3({
    abi: balancerRelayerAbi,
    functionName: "vaultActionsQueryMulticall",
    data
  });
  const peekedValue = decodeAbiParameters3(
    [{ type: "uint256" }],
    result[result.length - 1]
  )[0];
  return {
...
import {
  ...
  encodeFunctionData as encodeFunctionData40,
  zeroAddress as zeroAddress13
} from "viem";

// src/entities/swap/swaps/v2/auraBalSwaps/exitPool.ts
import {
  encodeAbiParameters as encodeAbiParameters6,
  encodeFunctionData as encodeFunctionData39,
...
function encodeExitData(token, userAddress, swapOpRef, limit, wethIsEth) {
  ...
  const poolKind = 1;
  const userData = encodeAbiParameters6(
    parseAbiParameters2("uint256, uint256, uint256"),
  ...
  const exitPoolData = encodeFunctionData39({
    abi: batchRelayerLibraryAbi,
    functionName: "exitPool",
    args: [
      balWethId,
...
async function querySwapExit(input) {
  ...
  const encodedMulticall = encodeFunctionData40({
    abi: balancerRelayerAbi,
    functionName: "vaultActionsQueryMulticall",
  ...
  const result = decodeFunctionResult4({
    abi: balancerRelayerAbi,
    functionName: "vaultActionsQueryMulticall",
    data
  });
  const peekedValue = decodeAbiParameters4(
    [{ type: "uint256" }],
    result[result.length - 1]
  )[0];
  return {
...
var AuraBalSwap = class {
  constructor(rpcUrl) {
    __publicField(this, "client");
    this.client = createPublicClient26({
      transport: http26(rpcUrl),
...
export {
  ...
  RemoveLiquidityNestedV2,
  RemoveLiquidityNestedV3,
  SECONDS_PER_YEAR,
  STABLE_POOL_FACTORY_BALANCER_V3,
  Slippage,
  Swap,
  SwapKind,
  TEST_API_ENDPOINT,
